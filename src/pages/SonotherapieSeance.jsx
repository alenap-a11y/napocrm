...
const INSTRUMENTS = ['Bols tibétains', 'Gong', 'Diapasons', 'Autre']
// Mêmes 7 zones/chakras que NapoÉnergie/Magnétiseur (identité réutilisée, pas
// de mesure par zone ici — juste une sélection des zones travaillées).
const ZONES = ['Muladhara (1er)', 'Svadhisthana (2e)', 'Manipura (3e)', 'Anahata (4e)', 'Vishuddha (5e)', 'Ajna (6e)', 'Sahasrara (7e)']
...
export default function SonotherapieSeance() {
  const { id } = useParams()
  ...
  useEffect(() => {
    async function load() {
      const { data: s } = await supabase.from('fiches_sonotherapie')
        .select('*, clients(id, prenom, nom)').eq('id', id).single()
      if (!s) { navigate('/napo-sonotherapie'); return }
      setSeance({ ...s, instruments_utilises: s.instruments_utilises || [], zones_travaillees: s.zones_travaillees || [] })
      setClient(s.clients)
      const { data: hist } = await supabase.from('fiches_sonotherapie')
  ...
  async function handleSave() {
    setSaving(true)
    await supabase.from('fiches_sonotherapie').update({
      date_seance: seance.date_seance,
      heure_seance: seance.heure_seance,
      instruments_utilises: seance.instruments_utilises,
      zones_travaillees: seance.zones_travaillees,
  ...
  return (
    <div style={{ padding:'1.6rem 2rem', fontFamily:'inherit', maxWidth:1100 }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:'1.2rem' }}>
        <button onClick={() => navigate('/napo-sonotherapie')} style={{ background:'none', border:'none', cursor:'pointer', color:'var(--color-text-secondary)', fontSize:13, display:'flex', alignItems:'center', gap:4, padding:0 }}>
          <i className="ti ti-arrow-left" style={{ fontSize:15 }} />Retour
        </button>
        <span style={{ color:'var(--color-border-secondary)' }}>/</span>
        <i className="ti ti-wave-sine" style={{ fontSize:18, color:'var(--color-accent)' }} />
        ...
        <div style={{ marginLeft:'auto', display:'flex', gap:8, alignItems:'center' }}>
          ...
          <button onClick={() => navigate(`/live/sonotherapie/${id}`)} style={{ display:'flex', alignItems:'center', gap:6, padding:'8px 18px', borderRadius:8, border:'0.5px solid var(--color-border-secondary)', background:'var(--color-background-secondary)', color:'var(--color-text-primary)', fontSize:13, fontWeight:600, cursor:'pointer' }}>
      ...
      <div style={{ background:'var(--color-background-secondary)', border:'0.5px solid var(--color-border-tertiary)', borderRadius:14, padding:'14px 16px', marginBottom:14 }}>
        ...
        <div>
          ...
          <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
            {historique.map(h => (
              <span key={h.id} onClick={() => navigate(`/napo-sonotherapie/${h.id}`)}
      ...
      {/* Instruments */}
      <div style={{ background:'var(--color-background-secondary)', border:'0.5px solid var(--color-border-tertiary)', borderRadius:14, padding:'14px 16px', marginBottom:14 }}>
        <div style={{ fontSize:9, color:'var(--color-text-secondary)', textTransform:'uppercase', letterSpacing:'.05em', marginBottom:8 }}>Instruments utilisés</div>
        <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
          {INSTRUMENTS.map(inst => (
            <button key={inst} onClick={() => updateSeanceField('instruments_utilises', toggleInArray(seance.instruments_utilises, inst))}
              style={{ fontSize:11, padding:'5px 10px', borderRadius:6, cursor:'pointer', border:'0.5px solid var(--color-border-secondary)', background:seance.instruments_utilises.includes(inst) ? '#4F46E5' : 'var(--color-background-primary)', color:seance.instruments_utilises.includes(inst) ? '#fff' : 'var(--color-text-secondary)' }}>
              {inst}
            </button>
          ))}
        </div>
      ...
      {/* Zones */}
      <div style={{ background:'var(--color-background-secondary)', border:'0.5px solid var(--color-border-tertiary)', borderRadius:14, padding:'14px 16px', marginBottom:14 }}>
        <div style={{ fontSize:9, color:'var(--color-text-secondary)', textTransform:'uppercase', letterSpacing:'.05em', marginBottom:8 }}>Zones travaillées</div>
        <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
          {ZONES.map(zone => (
            <button key={zone} onClick={() => updateSeanceField('zones_travaillees', toggleInArray(seance.zones_travaillees, zone))}
              style={{ fontSize:11, padding:'5px 10px', borderRadius:6, cursor:'pointer', border:'0.5px solid var(--color-border-secondary)', background:seance.zones_travaillees.includes(zone) ? '#4F46E5' : 'var(--color-background-primary)', color:seance.zones_travaillees.includes(zone) ? '#fff' : 'var(--color-text-secondary)' }}>
              {zone}
            </button>
          ))}
        </div>
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
        <div style={{ background:'var(--color-background-secondary)', border:'0.5px solid var(--color-border-tertiary)', borderRadius:14, padding:'14px 16px' }}>
          <div style={{ fontSize:9, color:'var(--color-text-secondary)', textTransform:'uppercase', letterSpacing:'.05em', marginBottom:6 }}>Ressenti</div>
          <textarea value={seance.ressenti || ''} onChange={e => updateSeanceField('ressenti', e.target.value)}
            rows={3} placeholder="Ressenti pendant/après la séance..."
