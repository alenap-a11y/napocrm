...
export default function Magnetisme() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const [{ data: s }, { data: c }] = await Promise.all([
        supabase.from('fiches_magnetisme').select('*, clients(prenom, nom)').eq('user_id', user.id).order('date_seance', { ascending: false }),
  ...
  async function handleNouvelle(clientId) {
    ...
    const { data, error } = await supabase.from('fiches_magnetisme').insert({
      user_id: user.id,
      client_id: clientId,
    ...
    if (!error && data) navigate(`/magnetisme/${data.id}`)
  }
    ...
    <div style={{ padding:'1.6rem 2rem', fontFamily:'inherit' }}>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'1.2rem' }}>
        <div style={{ display:'flex', alignItems:'center', gap:12 }}>
          <i className="ti ti-hand-stop" style={{ fontSize:24, color:'var(--color-accent)' }} />
          <div>
            <div style={{ fontSize:22, fontWeight:600, color:'var(--color-text-primary)' }}>Napo-Magnétiseur</div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:12, marginBottom:18 }}>
        <StatCard icon="ti-hand-stop" iconBg="#FBEAF0" iconColor="#993556" label="Total séances"  value={seances.length} />
          ...
          <div style={{ background:'var(--color-background-secondary)', borderRadius:12, overflow:'hidden', border:'0.5px solid var(--color-border-tertiary)' }}>
            ...
            {loading ? (
              ...
              <div style={{ padding:'40px', textAlign:'center', color:'var(--color-text-secondary)', fontSize:13 }}>
                <i className="ti ti-hand-stop" style={{ fontSize:28, display:'block', marginBottom:8 }} />
                Aucune séance magnétisme enregistrée
              </div>
            ) : filtered.map((s, idx) => (
              <div key={s.id} onClick={() => navigate(`/magnetisme/${s.id}`)}
