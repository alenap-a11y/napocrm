        ...
        {sent ? (
          <div style={{ textAlign: 'center', padding: '4rem 1rem', background: '#fff', borderRadius: 14, border: '1px solid #E5E7EB' }}>
            ...
            <div style={{ fontSize: 14, color: '#6B7280', lineHeight: 1.7 }}>
              Votre demande pour le <strong>{formatDateLong(selectedSlot.date_seance)}</strong> à <strong>{selectedSlot.heure_seance?.slice(0,5)}</strong> a bien été envoyée.<br/>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, alignItems: 'start' }}>
            ...
            <div style={{ background: '#fff', borderRadius: 14, border: '1px solid #E5E7EB', overflow: 'hidden' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 3, padding: '0 8px 12px' }}>
                {allCells.map((day, i) => {
                  ...
                  const isSelected = selectedSlot?.date_seance === ymd
                  const isToday = ymd === toYMD(new Date())
                  return (
                    <div key={ymd} onClick={() => { if (!isPast && hasSlots) setPopupDate(ymd) }}
                      style={{
                        ...
                        border: isSelected ? '2px solid #0F6E56' : isToday ? '1.5px solid #0F6E56' : hasSlots && !isPast ? '1px solid #BBF7D0' : '1px solid transparent',
                      }}>
              ...
              {selectedSlot && (
                <div style={{ margin: '0 12px 12px', padding: '10px 14px', borderRadius: 8, background: '#E1F5EE', border: '1px solid #BBF7D0', fontSize: 12, fontWeight: 600, color: '#0F6E56', textAlign: 'center' }}>
                  ✓ {formatDateLong(selectedSlot.date_seance)} à {selectedSlot.heure_seance?.slice(0,5)}
            ...
            <div style={{ background: '#fff', borderRadius: 14, border: `1.5px solid ${selectedSlot ? '#0F6E56' : '#E5E7EB'}`, padding: '20px', transition: 'border-color 0.2s' }}>
              ...
              <button onClick={submit} disabled={sending || !form.prenom || !form.nom || !form.email || !form.telephone || !selectedSlot}
                ...
                {sending ? 'Envoi...' : selectedSlot ? `✓ Confirmer le ${formatDateLong(selectedSlot.date_seance)} à ${selectedSlot.heure_seance?.slice(0,5)}` : 'Choisir un créneau'}
              </button>
      ...
      {popupDate && (
        <div onClick={() => setPopupDate(null)}
          ...
          <div onClick={e => e.stopPropagation()}
            style={{ background: '#fff', borderRadius: 16, padding: '28px 24px', width: 320, boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginBottom: 20 }}>
              {(slotsByDate[popupDate] || []).map(slot => (
                <button key={slot.id} onClick={() => { setSelectedSlot(slot); setPopupDate(null) }}
                  style={{ padding: '12px 20px', borderRadius: 10, border: '2px solid #BBF7D0', background: '#E1F5EE', color: '#0F6E56', fontSize: 15, fontWeight: 700, cursor: 'pointer', transition: 'all 0.1s' }}
                  onMouseEnter={e => { e.target.style.background = '#0F6E56'; e.target.style.color = '#fff' }}
                  onMouseLeave={e => { e.target.style.background = '#E1F5EE'; e.target.style.color = '#0F6E56' }}>
                  {slot.heure_seance?.slice(0,5)}
                  {slot.duree_minutes && <div style={{ fontSize: 10, fontWeight: 400, marginTop: 2 }}>{slot.duree_minutes} min</div>}
                </button>
              ))}
            </div>
