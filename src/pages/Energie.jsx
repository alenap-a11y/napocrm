...
export default function Energie() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const [{ data: s }, { data: c }] = await Promise.all([
        supabase.from('energie_seances').select('*, clients(prenom, nom)').eq('user_id', user.id).order('date_seance', { ascending: false }),
  ...
  async function handleNouvelle(clientId) {
    ...
    const { data, error } = await supabase.from('energie_seances').insert({
      user_id: user.id,
      client_id: clientId,
    ...
    if (!error && data) navigate(`/energie/${data.id}`)
  }
    ...
    <div style={{ padding:'1.6rem 2rem', fontFamily:'inherit' }}>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'1.2rem' }}>
        <div style={{ display:'flex', alignItems:'center', gap:12 }}>
          <i className="ti ti-sparkles" style={{ fontSize:24, color:'var(--color-accent)' }} />
          <div>
            <div style={{ fontSize:22, fontWeight:600, color:'var(--color-text-primary)' }}>NapoÉnergie</div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:12, marginBottom:18 }}>
        <StatCard icon="ti-sparkles"   iconBg="#FBEAF0" iconColor="#993556" label="Total séances"    value={seances.length} />
        ...
        <StatCard icon="ti-chart-line" iconBg="#EEEDFE" iconColor="#534AB7" label="Chakras / séance" value="7" />
      </div>
        ...
        <>
          ...
          <div style={{ background:'var(--color-background-secondary)', borderRadius:12, overflow:'hidden', border:'0.5px solid var(--color-border-tertiary)' }}>
            ...
            {loading ? (
              ...
              <div key={s.id} onClick={() => navigate(`/energie/${s.id}`)}
