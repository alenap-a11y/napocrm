...
const MOCK = [
  { id: 1,  prenom: 'Marie',   nom: 'Joubert',  type_seance: 'Sophrologie',  date: '2026-05-27', heure: '10:00', duree: '60',  prix: 60,  observations_cles: ['Stress','Sommeil'],   notes: 'Bon relâchement musculaire. Retour positif sur la respiration.' },
  { id: 2,  prenom: 'Pierre',  nom: 'Laurent',  type_seance: 'Coaching',     date: '2026-05-27', heure: '14:30', duree: '45',  prix: 80,  observations_cles: ['Anxiété'],            notes: 'Travail sur les objectifs Q3. Plan d\'action établi.' },
  { id: 3,  prenom: 'Sophie',  nom: 'Caron',    type_seance: 'Naturopathie', date: '2026-05-26', heure: '09:00', duree: '90',  prix: 95,  observations_cles: ['Fatigue','Sommeil'],  notes: 'Bilan alimentaire complet. Compléments recommandés.' },
  { id: 4,  prenom: 'Camille', nom: 'Dumas',    type_seance: 'Sophrologie',  date: '2026-05-24', heure: '11:00', duree: '60',  prix: 60,  observations_cles: ['Stress'],             notes: 'Première séance. Très réceptive aux exercices.' },
  { id: 5,  prenom: 'Lucie',   nom: 'Martin',   type_seance: 'Coaching',     date: '2026-05-22', heure: '15:00', duree: '45',  prix: 80,  observations_cles: ['Douleur'],            notes: 'Suivi post-burn out. Reprise progressive.' },
  { id: 6,  prenom: 'Paul',    nom: 'Renard',   type_seance: 'Naturopathie', date: '2026-05-20', heure: '10:00', duree: '60',  prix: 75,  observations_cles: ['Fatigue'],            notes: 'Protocole détox 21 jours démarré.' },
  { id: 7,  prenom: 'Anna',    nom: 'Leblanc',  type_seance: 'Sophrologie',  date: '2026-05-18', heure: '14:00', duree: '60',  prix: 60,  observations_cles: ['Anxiété','Stress'],   notes: 'Travail sur la visualisation positive.' },
  { id: 8,  prenom: 'Marie',   nom: 'Joubert',  type_seance: 'Sophrologie',  date: '2026-05-13', heure: '10:00', duree: '60',  prix: 60,  observations_cles: ['Sommeil'],            notes: 'Nette amélioration du sommeil signalée.' },
  { id: 9,  prenom: 'Sophie',  nom: 'Caron',    type_seance: 'Naturopathie', date: '2026-05-10', heure: '09:30', duree: '90',  prix: 95,  observations_cles: ['Fatigue'],            notes: 'Contrôle des résultats sanguins. Magnésium augmenté.' },
  { id: 10, prenom: 'Thomas',  nom: 'Bernard',  type_seance: 'Énergie',      date: '2026-05-08', heure: '11:30', duree: '60',  prix: 70,  observations_cles: ['Lombaires'],          notes: 'Blocage L4-L5 libéré. Exercices donnés.' },
  { id: 11, prenom: 'Lucie',   nom: 'Martin',   type_seance: 'Coaching',     date: '2026-05-05', heure: '16:00', duree: '45',  prix: 80,  observations_cles: [],                     notes: 'Bilan mensuel. Progression notable.' },
  { id: 12, prenom: 'Paul',    nom: 'Renard',   type_seance: 'Naturopathie', date: '2026-04-28', heure: '10:00', duree: '60',  prix: 75,  observations_cles: ['Fatigue'],            notes: 'Mi-parcours détox. Résultats encourageants.' },
]
...
function toCSV(seances) {
  ...
  const rows = seances.map(s =>
    `${s.prenom},${s.nom},${s.type_seance},${s.date},${s.heure},${s.duree},${s.prix},"${(s.observations_cles||[]).join('|')}","${(s.notes||'').replace(/"/g, "'").replace(/\n/g, ' ')}"`
  )
...
function parseCSV(text) {
  ...
  return lines.map((line, i) => {
    ...
    return {
      ...
      heure: c[4] || '09:00',
      duree: c[5] || '60',
      prix: parseFloat(c[6]) || 0,
      observations_cles: c[7] ? c[7].split('|').filter(Boolean) : [],
      notes: c[8] || '',
    }
...
export default function Seances() {
  ...
  const EMPTY_FORM = { prenom: '', nom: '', type_seance: 'Sophrologie', date: new Date().toISOString().slice(0,10), heure: '10:00', duree: '60', prix: '', observations_cles: '', notes: '' }
  ...
  // ── Filtres ──────────────────────────────────────────────────────────────
  const moisDispos = [...new Set(seances.map(s => s.date?.slice(0, 7)).filter(Boolean))].sort().reverse()
  ...
  const filtered = seances.filter(s => {
    ...
    const matchMois = filterMois === 'tous' || (s.date || '').startsWith(filterMois)
    return matchSearch && matchType && matchMois
  })

  // ── Stats ────────────────────────────────────────────────────────────────
  const currentMonth = new Date().toISOString().slice(0, 7)
  const seancesMois  = seances.filter(s => (s.date || '').startsWith(currentMonth))
  const revenuTotal  = seances.reduce((s, x) => s + (parseFloat(x.prix) || 0), 0)
  const revenuMois   = seancesMois.reduce((s, x) => s + (parseFloat(x.prix) || 0), 0)
  const dureeAvg     = seances.length
    ? Math.round(seances.reduce((s, x) => s + (parseInt(x.duree) || 0), 0) / seances.length)
    : 0
  ...
  async function handleSave() {
    ...
    try {
      ...
      const toInsert = seances.map(s => ({ ...s, user_id: user.id }))
      const { error } = await supabase.from('seances').upsert(toInsert, { onConflict: 'id' })
  ...
  function openEdit(s) {
    setEditForm({
      ...
      duree: s.duree || '60',
      prix: s.prix || '',
      observations_cles: (s.observations_cles || []).join(', '),
      notes: s.notes || '',
    })
  ...
  function saveEditDetail() {
    const tags = editForm.observations_cles.split(',').map(t => t.trim()).filter(Boolean)
    const updated = { ...detail, ...editForm, prix: parseFloat(editForm.prix) || 0, observations_cles: tags }
  ...
  return (
    <div style={{ padding: '1.6rem 2rem', fontFamily: 'inherit' }}>
      ...
      {/* ── Tabs ── */}
      <div style={{ display: 'flex', borderBottom: '0.5px solid var(--color-border-tertiary)', marginBottom: 16, gap: 0 }}>
        {[
      ...
      {activeTab === 'historique' && (
        <>
          ...
          <div style={{ background: 'var(--color-background-secondary)', borderRadius: 12, overflow: 'hidden', border: '0.5px solid var(--color-border-tertiary)' }}>
            {/* Header */}
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 140px 90px 70px 80px 90px 36px', gap: 0, padding: '8px 16px', borderBottom: '0.5px solid var(--color-border-tertiary)' }}>
            ...
            {filtered.length === 0 ? (
              ...
              const tags = s.observations_cles || []
              return (
                <div
                  key={s.id}
                  onClick={() => setDetail(s)}
                  style={{ display: 'grid', gridTemplateColumns: '1fr 140px 90px 70px 80px 90px 36px', gap: 0, padding: '11px 16px', alignItems: 'center', cursor: 'pointer', borderBottom: idx < filtered.length - 1 ? '0.5px solid var(--color-border-tertiary)' : 'none', transition: 'background .1s' }}
                ...
                >
                  <div>
                    ...
                    {s.heure && <div style={{ fontSize: 11, color: 'var(--color-text-secondary)' }}>{s.heure}</div>}
                  </div>
                  <div style={{ fontSize: 12, color: 'var(--color-text-secondary)' }}>{fmtDate(s.date)}</div>
                  ...
                  <div style={{ fontSize: 12, color: 'var(--color-text-secondary)' }}>{s.duree} min</div>
                  <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--color-text-primary)', textAlign: 'right' }}>{fmt(s.prix)}</div>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                    <button onClick={e => { e.stopPropagation(); downloadCSV(toCSV([s]), `seance-${s.prenom}-${s.date}.csv`) }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-text-secondary)', padding: 4 }} title="Exporter">
          ...
          {filtered.length > 0 && (
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 10, fontSize: 13, color: 'var(--color-text-secondary)', gap: 6 }}>
              ...
              <span style={{ fontWeight: 600, color: 'var(--color-text-primary)' }}>
                {fmt(filtered.reduce((s, x) => s + (parseFloat(x.prix) || 0), 0))} encaissés
              </span>
            </div>
          )}
        </>
      ...
      {activeTab === 'nouvelle' && (
        <div style={{ maxWidth: 600 }}>
          ...
          <div style={{ background: 'var(--color-background-secondary)', borderRadius: 14, border: '0.5px solid var(--color-border-tertiary)', padding: '28px 28px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 14 }}>
              ...
              <Field label="Durée (min)">
                <input type="number" value={form.duree} onChange={e => setForm(f => ({...f, duree: e.target.value}))} placeholder="60" min="1" style={inputStyle} />
              </Field>
              <Field label="Prix (€)">
                <input type="number" value={form.prix} onChange={e => setForm(f => ({...f, prix: e.target.value}))} placeholder="60" min="0" style={inputStyle} />
              </Field>
              <Field label="Tags (séparés par ,)">
                <input value={form.observations_cles} onChange={e => setForm(f => ({...f, observations_cles: e.target.value}))} placeholder="Stress, Sommeil…" style={inputStyle} />
              </Field>
            </div>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 22, justifyContent: 'flex-end' }}>
              ...
              <button onClick={() => {
                ...
                if (!form.date) { setFormMsg('Date requise.'); return }
                const tags = form.observations_cles.split(',').map(t => t.trim()).filter(Boolean)
                const newSeance = { ...form, id: Date.now(), prix: parseFloat(form.prix) || 0, observations_cles: tags }
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, maxWidth: 800 }}>
          ...
          <div style={{ padding: '22px', borderRadius: 12, background: 'var(--color-background-secondary)', border: '0.5px solid var(--color-border-tertiary)' }}>
            ...
            <div style={{ fontSize: 13, color: 'var(--color-text-secondary)', lineHeight: 1.6, marginBottom: 16 }}>
              Téléchargez toutes vos séances dans un fichier CSV. Les colonnes exportées sont : Prénom, Nom, Type, Date, Heure, Durée, Prix, Tags, Notes.
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              <button onClick={() => { const m = seances.filter(s => (s.date||'').startsWith(currentMonth)); downloadCSV(toCSV(m), `seances-${currentMonth}.csv`) }} style={exportBtnStyle('#185FA5', '#E6F1FB')}>
          ...
          <div style={{ padding: '22px', borderRadius: 12, background: 'var(--color-background-secondary)', border: '0.5px solid var(--color-border-tertiary)' }}>
            ...
            <div style={{ fontSize: 13, color: 'var(--color-text-secondary)', lineHeight: 1.6, marginBottom: 16 }}>
              Importez des séances depuis un fichier CSV. Les colonnes attendues : <code style={{ fontSize: 11, background: 'var(--color-background-primary)', padding: '1px 5px', borderRadius: 4 }}>Prénom, Nom, Type, Date, Heure, Durée, Prix, Tags, Notes</code>
            </div>
          ...
          <div style={{ padding: '22px', borderRadius: 12, background: 'var(--color-background-secondary)', border: '0.5px solid var(--color-border-tertiary)', gridColumn: '1 / -1' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16 }}>
              <div style={{ fontSize: 13, color: 'var(--color-text-secondary)', lineHeight: 1.6 }}>
                Toutes les séances créées via "+ Nouvelle séance" sont sauvegardées automatiquement. Utilisez ce bouton pour forcer la synchronisation après un import CSV ou une modification manuelle.
              </div>
      ...
      {detail && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}
          ...
          <div style={{ background: 'var(--color-background-primary)', borderRadius: 16, width: 500, maxWidth: '95vw', maxHeight: '90vh', overflowY: 'auto', padding: 28, boxShadow: '0 20px 60px rgba(0,0,0,0.3)' }}>
            ...
            ) : (() => { const tc = TYPE_COLOR[detail.type_seance] || TYPE_COLOR['Autre']; return (
              <span style={{ fontSize: 11, fontWeight: 600, background: tc.bg, color: tc.color, padding: '4px 10px', borderRadius: 20, display: 'inline-block', marginBottom: 18 }}>{detail.type_seance}</span>
            )})()}

            {/* Champs date/heure/durée/prix */}
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 16 }}>
              {editingDetail ? (
                <>
                  <MField label="Date">   <input type="date"   value={editForm.date}  onChange={e => setEditForm(f => ({ ...f, date:  e.target.value }))} style={mInp} /></MField>
                  <MField label="Heure">  <input type="time"   value={editForm.heure} onChange={e => setEditForm(f => ({ ...f, heure: e.target.value }))} style={mInp} /></MField>
                  <MField label="Durée (min)"><input type="number" value={editForm.duree} onChange={e => setEditForm(f => ({ ...f, duree: e.target.value }))} min={1} style={mInp} /></MField>
                  <MField label="Prix (€)">   <input type="number" value={editForm.prix}  onChange={e => setEditForm(f => ({ ...f, prix:  e.target.value }))} min={0} style={mInp} /></MField>
                </>
              ) : (
                <>
                  <InfoRow label="Date"  value={fmtDate(detail.date)} />
                  <InfoRow label="Heure" value={detail.heure || '—'} />
                  <InfoRow label="Durée" value={`${detail.duree} min`} />
                  <InfoRow label="Prix"  value={fmt(detail.prix)} />
                </>
              )}
            </div>
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              {editingDetail ? (
                <input value={editForm.observations_cles} onChange={e => setEditForm(f => ({ ...f, observations_cles: e.target.value }))}
                  placeholder="Stress, Sommeil, Anxiété…" style={mInp} />
              ) : (
                (detail.observations_cles || []).length > 0
                  ? <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
                      {detail.observations_cles.map(t => (
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              {editingDetail ? (
                ...
                <>
                  ...
                  <button onClick={() => downloadCSV(toCSV([detail]), `seance-${detail.prenom}-${detail.date}.csv`)}
