  ...
  useEffect(() => {
    async function fetchUsers() {
      ...
      const { data, error } = await supabase
        ...
        .select('id, prenom, slug, is_admin, agenda_public, welcomed_at, email, last_sign_in_at, created_at')
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ borderBottom: '0.5px solid var(--color-border-tertiary)' }}>
              {['#', 'Utilisateur', 'Email', 'Rôle', 'Statut / Dernière connexion', ''].map((h, i) => (
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={6} style={{ padding: '2rem', textAlign: 'center', fontSize: '13px', color: 'var(--color-text-secondary)' }}>Chargement...</td></tr>
            ) : paginated.length === 0 ? (
              <tr><td colSpan={6} style={{ padding: '2rem', textAlign: 'center', fontSize: '13px', color: 'var(--color-text-secondary)' }}>Aucun utilisateur trouvé</td></tr>
            ) : paginated.map((u, i) => (
              <tr key={u.id}
                ...
                <td style={{ padding: '12px 14px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    ...
                    <span style={{ fontSize: '13px', fontWeight: 500 }}>{u.prenom || '—'}</span>
                  </div>
                </td>
                <td style={{ padding: '12px 14px', fontSize: '12px', color: 'var(--color-text-secondary)' }}>
                  {u.email || '—'}
