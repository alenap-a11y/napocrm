...
export default function Hypnotherapie() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const [{ data: s }, { data: c }] = await Promise.all([
        supabase.from('fiches_hypnotherapie').select('*, clients(prenom, nom)').eq('user_id', user.id).order('date_seance', { ascending: false }),
  ...
  async function handleNouvelle(clientId) {
    ...
    const { data, error } = await supabase.from('fiches_hypnotherapie').insert({
      user_id: user.id,
      client_id: clientId,
    ...
    if (!error && data) navigate(`/napo-hypnotherapie/${data.id}`)
  }
    ...
    <div style={{ padding:'1.6rem 2rem', fontFamily:'inherit' }}>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'1.2rem' }}>
        <div style={{ display:'flex', alignItems:'center', gap:12 }}>
          <i className="ti ti-spiral" style={{ fontSize:24, color:'var(--color-accent)' }} />
          <div>
            <div style={{ fontSize:22, fontWeight:600, color:'var(--color-text-primary)' }}>Napo-Hypnothérapie</div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:12, marginBottom:18 }}>
        <StatCard icon="ti-spiral"   iconBg="#EEF2FF" iconColor="#4F46E5" label="Total séances"  value={seances.length} />
        ...
        <>
          ...
          <div style={{ background:'var(--color-background-secondary)', borderRadius:12, overflow:'hidden', border:'0.5px solid var(--color-border-tertiary)' }}>
            <div style={{ display:'grid', gridTemplateColumns:'1.5fr 1fr 80px 1fr 80px', padding:'8px 16px', borderBottom:'0.5px solid var(--color-border-tertiary)' }}>
              {['Client','Date','Séance N°','Induction',''].map((h,i) => (
            ...
            {loading ? (
              ...
              <div style={{ padding:'40px', textAlign:'center', color:'var(--color-text-secondary)', fontSize:13 }}>
                <i className="ti ti-spiral" style={{ fontSize:28, display:'block', marginBottom:8 }} />
                Aucune séance hypnothérapie enregistrée
              </div>
            ) : filtered.map((s, idx) => (
              <div key={s.id} onClick={() => navigate(`/napo-hypnotherapie/${s.id}`)}
                ...
                <div style={{ fontSize:11, color:'var(--color-text-secondary)' }}>{s.type_induction || '—'}</div>
