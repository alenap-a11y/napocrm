...
const PRATIQUES = ['Soin', 'Voyage chamanique', 'Autre']
...
export default function ChamanismeSeance() {
  const { id } = useParams()
  ...
  useEffect(() => {
    async function load() {
      const { data: s } = await supabase.from('fiches_chamanisme')
        .select('*, clients(id, prenom, nom)').eq('id', id).single()
      if (!s) { navigate('/napo-chamanisme'); return }
      setSeance(s)
      setClient(s.clients)
      const { data: hist } = await supabase.from('fiches_chamanisme')
  ...
  async function handleSave() {
    setSaving(true)
    await supabase.from('fiches_chamanisme').update({
      date_seance: seance.date_seance,
      heure_seance: seance.heure_seance,
      type_pratique: seance.type_pratique,
      elements_utilises: seance.elements_utilises,
      ressenti_message_recu: seance.ressenti_message_recu,
      bilan: seance.bilan,
  ...
  return (
    <div style={{ padding:'1.6rem 2rem', fontFamily:'inherit', maxWidth:1100 }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:'1.2rem' }}>
        <button onClick={() => navigate('/napo-chamanisme')} style={{ background:'none', border:'none', cursor:'pointer', color:'var(--color-text-secondary)', fontSize:13, display:'flex', alignItems:'center', gap:4, padding:0 }}>
          <i className="ti ti-arrow-left" style={{ fontSize:15 }} />Retour
        </button>
        <span style={{ color:'var(--color-border-secondary)' }}>/</span>
        <i className="ti ti-feather" style={{ fontSize:18, color:'var(--color-accent)' }} />
        ...
        <div style={{ marginLeft:'auto', display:'flex', gap:8, alignItems:'center' }}>
          ...
          <button onClick={() => navigate(`/live/chamanisme/${id}`)} style={{ display:'flex', alignItems:'center', gap:6, padding:'8px 18px', borderRadius:8, border:'0.5px solid var(--color-border-secondary)', background:'var(--color-background-secondary)', color:'var(--color-text-primary)', fontSize:13, fontWeight:600, cursor:'pointer' }}>
      ...
      <div style={{ background:'var(--color-background-secondary)', border:'0.5px solid var(--color-border-tertiary)', borderRadius:14, padding:'14px 16px', marginBottom:14 }}>
        ...
        <div style={{ display:'flex', gap:16, flexWrap:'wrap', marginBottom:12, alignItems:'center' }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:6 }}>
            <span style={{ fontSize:10, color:'var(--color-text-secondary)', textTransform:'uppercase', letterSpacing:'.04em' }}>Pratique</span>
            <select value={seance.type_pratique || ''} onChange={e => updateSeanceField('type_pratique', e.target.value)} style={{ ...inp, width:'auto' }}>
        ...
        <div>
          ...
          <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
            {historique.map(h => (
              <span key={h.id} onClick={() => navigate(`/napo-chamanisme/${h.id}`)}
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
        <div style={{ background:'var(--color-background-secondary)', border:'0.5px solid var(--color-border-tertiary)', borderRadius:14, padding:'14px 16px' }}>
          <div style={{ fontSize:9, color:'var(--color-text-secondary)', textTransform:'uppercase', letterSpacing:'.05em', marginBottom:6 }}>Éléments utilisés</div>
          <textarea value={seance.elements_utilises || ''} onChange={e => updateSeanceField('elements_utilises', e.target.value)}
            rows={2} placeholder="ex: tambour, plumes, encens..."
            style={{ ...inp, resize:'none', lineHeight:1.6 }} />
        </div>

        <div style={{ background:'var(--color-background-secondary)', border:'0.5px solid var(--color-border-tertiary)', borderRadius:14, padding:'14px 16px' }}>
          <div style={{ fontSize:9, color:'var(--color-text-secondary)', textTransform:'uppercase', letterSpacing:'.05em', marginBottom:6 }}>Ressenti / message reçu</div>
          <textarea value={seance.ressenti_message_recu || ''} onChange={e => updateSeanceField('ressenti_message_recu', e.target.value)}
            rows={4} placeholder="Contenu reçu pendant la pratique..."
            style={{ ...inp, resize:'none', lineHeight:1.6 }} />
        </div>
