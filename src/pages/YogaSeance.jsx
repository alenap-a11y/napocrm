...
const TYPES = ['Individuelle', 'Cours collectif', 'Autre']
...
export default function YogaSeance() {
  const { id } = useParams()
  ...
  useEffect(() => {
    async function load() {
      const { data: s } = await supabase.from('fiches_yoga')
        .select('*, clients(id, prenom, nom)').eq('id', id).single()
      if (!s) { navigate('/napo-yoga'); return }
      setSeance(s)
      setClient(s.clients)
      const { data: hist } = await supabase.from('fiches_yoga')
  ...
  async function handleSave() {
    setSaving(true)
    await supabase.from('fiches_yoga').update({
      date_seance: seance.date_seance,
      heure_seance: seance.heure_seance,
      type_seance: seance.type_seance,
      postures_exercices: seance.postures_exercices,
      ressenti: seance.ressenti,
      notes_suivi: seance.notes_suivi,
  ...
  return (
    <div style={{ padding:'1.6rem 2rem', fontFamily:'inherit', maxWidth:1100 }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:'1.2rem' }}>
        <button onClick={() => navigate('/napo-yoga')} style={{ background:'none', border:'none', cursor:'pointer', color:'var(--color-text-secondary)', fontSize:13, display:'flex', alignItems:'center', gap:4, padding:0 }}>
          <i className="ti ti-arrow-left" style={{ fontSize:15 }} />Retour
        </button>
        <span style={{ color:'var(--color-border-secondary)' }}>/</span>
        <i className="ti ti-yoga" style={{ fontSize:18, color:'var(--color-accent)' }} />
        ...
        <div style={{ marginLeft:'auto', display:'flex', gap:8, alignItems:'center' }}>
          ...
          <button onClick={() => navigate(`/live/yoga/${id}`)} style={{ display:'flex', alignItems:'center', gap:6, padding:'8px 18px', borderRadius:8, border:'0.5px solid var(--color-border-secondary)', background:'var(--color-background-secondary)', color:'var(--color-text-primary)', fontSize:13, fontWeight:600, cursor:'pointer' }}>
      ...
      <div style={{ background:'var(--color-background-secondary)', border:'0.5px solid var(--color-border-tertiary)', borderRadius:14, padding:'14px 16px', marginBottom:14 }}>
        ...
        <div style={{ display:'flex', gap:16, flexWrap:'wrap', marginBottom:12, alignItems:'center' }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:6 }}>
            ...
            <select value={seance.type_seance || ''} onChange={e => updateSeanceField('type_seance', e.target.value)} style={{ ...inp, width:'auto' }}>
        ...
        <div style={{ marginBottom:12 }}>
          ...
          <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
            {historique.map(h => (
              <span key={h.id} onClick={() => navigate(`/napo-yoga/${h.id}`)}
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
        <div style={{ background:'var(--color-background-secondary)', border:'0.5px solid var(--color-border-tertiary)', borderRadius:14, padding:'14px 16px' }}>
          <div style={{ fontSize:9, color:'var(--color-text-secondary)', textTransform:'uppercase', letterSpacing:'.05em', marginBottom:6 }}>Postures / exercices travaillés</div>
          <textarea value={seance.postures_exercices || ''} onChange={e => updateSeanceField('postures_exercices', e.target.value)}
            rows={4} placeholder="Postures, enchaînements, respirations..."
            style={{ ...inp, resize:'none', lineHeight:1.6 }} />
        </div>

        <div style={{ background:'var(--color-background-secondary)', border:'0.5px solid var(--color-border-tertiary)', borderRadius:14, padding:'14px 16px' }}>
          <div style={{ fontSize:9, color:'var(--color-text-secondary)', textTransform:'uppercase', letterSpacing:'.05em', marginBottom:6 }}>Ressenti du pratiquant</div>
        ...
        <div style={{ background:'var(--color-background-secondary)', border:'0.5px solid var(--color-border-tertiary)', borderRadius:14, padding:'14px 16px' }}>
          <div style={{ fontSize:9, color:'var(--color-text-secondary)', textTransform:'uppercase', letterSpacing:'.05em', marginBottom:6 }}>Notes de suivi</div>
          <textarea value={seance.notes_suivi || ''} onChange={e => updateSeanceField('notes_suivi', e.target.value)}
            rows={3} placeholder="Progression, points de vigilance..."
            style={{ ...inp, resize:'none', lineHeight:1.6 }} />
        </div>
      </div>
