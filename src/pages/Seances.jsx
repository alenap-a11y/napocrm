...
function parseCSV(text) {
  ...
  return lines.map((line, i) => {
    ...
    return {
      id: Date.now() + i,
      prenom: c[0]||'', nom: c[1]||'', type_seance: c[2]||'Autre',
      date_seance: c[3]||'', heure_seance: c[4]||'09:00',
      duree_minutes: parseInt(c[5])||60, prix_euros: parseFloat(c[6])||0,
      tags: c[7] ? c[7].split('|').filter(Boolean) : [], notes: c[8]||'',
    }
...
export default function Seances() {
  ...
  const dureeAvg     = seances.length ? Math.round(seances.reduce((acc,x) => acc+(parseInt(x.duree_minutes)||0),0)/seances.length) : 0
  ...
  function openEdit(s) {
    setEditForm({
      prenom: s.prenom||'', nom: s.nom||'', type_seance: s.type_seance||'Sophrologie',
      date_seance: s.date_seance||'', heure_seance: s.heure_seance||'',
      duree_minutes: s.duree_minutes||'60', prix_euros: s.prix_euros||'',
        ...
        <>
          <div style={{ display:'flex', gap:10, marginBottom:14, flexWrap:'wrap' }}>
            <div style={{ position:'relative', flex:1, minWidth:200 }}>
              ...
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Rechercher client, type, notes…"
            ...
            <Sel value={filterMois} onChange={setFilterMois} options={[['tous','Tous les mois'],...moisDispos.map(m => { const [y,mo]=m.split('-'); return [m,`${MOIS[parseInt(mo)-1]} ${y}`] })]} />
          </div>
          ...
          <div style={{ background:'var(--color-background-secondary)', borderRadius:12, overflow:'hidden', border:'0.5px solid var(--color-border-tertiary)' }}>
            ...
            {filtered.length === 0 ? (
              <div style={{ padding:'40px', textAlign:'center', color:'var(--color-text-secondary)', fontSize:13 }}>
                <i className="ti ti-calendar-off" style={{ fontSize:32, display:'block', marginBottom:10 }} />Aucune séance trouvée
              </div>
            ) : filtered.map((s,idx) => {
              ...
              return (
                <div key={s.id} onClick={() => openDetail(s)}
                  ...
                  <div style={{ display:'flex', flexWrap:'wrap', gap:3 }}>
                    {tags.slice(0,2).map(t => <span key={t} style={{ fontSize:10, background:'var(--color-background-primary)', color:'var(--color-text-secondary)', padding:'1px 6px', borderRadius:20, border:'0.5px solid var(--color-border-tertiary)' }}>{t}</span>)}
                    {tags.length>2 && <span style={{ fontSize:10, color:'var(--color-text-secondary)' }}>+{tags.length-2}</span>}
                  </div>
                  <div style={{ display:'flex', justifyContent:'flex-end' }}>
                    <button onClick={e => { e.stopPropagation(); downloadCSV(toCSV([s]),`seance-${s.prenom}-${s.date_seance}.csv`) }} style={{ background:'none', border:'none', cursor:'pointer', color:'var(--color-text-secondary)', padding:4 }}>
          ...
          {filtered.length > 0 && (
            <div style={{ display:'flex', justifyContent:'flex-end', marginTop:10, fontSize:13, color:'var(--color-text-secondary)', gap:6 }}>
              ...
              <span style={{ fontWeight:600, color:'var(--color-text-primary)' }}>{fmt(filtered.reduce((acc,x)=>acc+(parseFloat(x.prix_euros)||0),0))} encaissés</span>
            </div>
          )}
        </>
      )}
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16, maxWidth:800 }}>
          <div style={{ padding:'22px', borderRadius:12, background:'var(--color-background-secondary)', border:'0.5px solid var(--color-border-tertiary)' }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
              <button onClick={() => downloadCSV(toCSV(seances),'seances-napocrm.csv')} style={xBtn('#0F6E56','#E1F5EE')}><i className="ti ti-table-export" style={{ fontSize:15 }} /> Exporter tout ({seances.length})</button>
              <button onClick={() => { const m=seances.filter(s=>(s.date_seance||'').startsWith(currentMonth)); downloadCSV(toCSV(m),`seances-${currentMonth}.csv`) }} style={xBtn('#185FA5','#E6F1FB')}><i className="ti ti-calendar-down" style={{ fontSize:15 }} /> Ce mois ({seancesMois.length})</button>
            </div>
          </div>
          <div style={{ padding:'22px', borderRadius:12, background:'var(--color-background-secondary)', border:'0.5px solid var(--color-border-tertiary)' }}>
            <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:14 }}>
            ...
            <button onClick={() => importRef.current.click()} style={xBtn('#534AB7','#EEEDFE')}><i className="ti ti-file-upload" style={{ fontSize:15 }} /> Choisir un fichier CSV</button>
          </div>
          <div style={{ padding:'22px', borderRadius:12, background:'var(--color-background-secondary)', border:'0.5px solid var(--color-border-tertiary)', gridColumn:'1/-1' }}>
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:16 }}>
              <div style={{ fontSize:13, color:'var(--color-text-secondary)', lineHeight:1.6 }}>Synchronise les données importées via CSV vers Supabase.</div>
              <button onClick={handleSave} disabled={saving} style={{ flexShrink:0, display:'flex', alignItems:'center', gap:8, padding:'10px 20px', borderRadius:8, border:'none', background:'#854F0B', color:'#fff', fontSize:13, fontWeight:600, cursor:saving?'default':'pointer', opacity:saving?.7:1 }}>
                <i className="ti ti-cloud-upload" style={{ fontSize:16 }} />{saving?'Sync…':'Synchroniser CSV'}
              </button>
            </div>
            {saveMsg && <div style={{ marginTop:10, padding:'8px 12px', borderRadius:8, background:saveMsg.startsWith('✓')?'#EAF3DE':'#FBEAF0', color:saveMsg.startsWith('✓')?'#3B6D11':'#993556', fontSize:13 }}>{saveMsg}</div>}
          </div>
        </div>
      )}

      {/* ══ MODAL DÉTAIL SÉANCE — FICHE 3 ONGLETS ══ */}
      {detail && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.45)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1000 }}
          ...
          <div style={{ background:'var(--color-background-primary)', borderRadius:16, width:560, maxWidth:'95vw', maxHeight:'90vh', overflowY:'auto', padding:28, boxShadow:'0 20px 60px rgba(0,0,0,0.3)' }}>
            ...
            {(() => {
              ...
              const tc  = TYPE_COLOR[src.type_seance] || TYPE_COLOR['Autre']
              return (
                <div style={{ display:'flex', alignItems:'center', gap:14, marginBottom:20 }}>
                  <div style={{ width:48, height:48, borderRadius:'50%', background:tc.bg, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                    <span style={{ fontSize:16, fontWeight:700, color:tc.color }}>{(src.prenom||'?')[0]}{(src.nom||'?')[0]}</span>
                  </div>
                  <div style={{ flex:1 }}>
                    {editingDetail ? (
                      <div style={{ display:'flex', gap:8 }}>
                    ...
                    <div style={{ marginTop:6 }}>
                      {editingDetail ? (
                        <select value={editForm.type_seance} onChange={e=>setEditForm(f=>({...f,type_seance:e.target.value}))} style={{ ...mInp, fontSize:11, padding:'2px 8px', width:'auto' }}>
                          {TYPES.slice(1).map(t=><option key={t} value={t}>{t}</option>)}
                        </select>
                      ) : (
                        <span style={{ fontSize:11, fontWeight:600, background:tc.bg, color:tc.color, padding:'2px 8px', borderRadius:20 }}>{detail.type_seance}</span>
                      )}
                    </div>
                  </div>
            ...
            {!editingDetail && (
              <div style={{ display:'flex', borderBottom:'0.5px solid var(--color-border-tertiary)', marginBottom:18 }}>
                {[['infos','Infos','ti-info-circle'],['historique','Historique','ti-history'],['notes','Notes','ti-notes']].map(([id,label,icon]) => (
            ...
            {(detailTab==='infos' || editingDetail) && (
              editingDetail ? (
                ...
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginBottom:16 }}>
                  {[
                    ['ti-calendar',    'Date',    fmtDate(detail.date_seance)],
                    ['ti-clock',       'Heure',   detail.heure_seance||'—'],
                    ['ti-hourglass',   'Durée',   `${detail.duree_minutes} min`],
                    ['ti-coin',        'Prix',    fmt(detail.prix_euros)],
                    ['ti-tag',         'Tags',    (detail.tags||[]).join(', ')||'—'],
                    ['ti-user',        'Client',  clientName(detail)],
                  ].map(([icon,label,val]) => (
            ...
            {detailTab==='historique' && !editingDetail && (() => {
              ...
              return autresSeances.length === 0 ? (
                <div style={{ textAlign:'center', padding:'30px', color:'var(--color-text-secondary)', fontSize:13 }}>
                  ...
                  Aucune autre séance pour ce client
                </div>
              ) : (
                <>
                  {autresSeances.map((s,idx) => {
                    const tc2 = TYPE_COLOR[s.type_seance]||TYPE_COLOR['Autre']
                    return (
                      <div key={s.id} style={{ display:'flex', gap:12, alignItems:'flex-start', padding:'10px 0', borderBottom:idx<autresSeances.length-1?'0.5px solid var(--color-border-tertiary)':'none' }}>
                        <div style={{ width:8, height:8, borderRadius:'50%', background:tc2.color, flexShrink:0, marginTop:5 }} />
                        <div style={{ flex:1 }}>
                          <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:3 }}>
                            <span style={{ fontSize:13, fontWeight:500, color:'var(--color-text-primary)' }}>{fmtDateCourt(s.date_seance)}</span>
                            {s.heure_seance && <span style={{ fontSize:11, color:'var(--color-text-secondary)' }}>{s.heure_seance}</span>}
                            <span style={{ fontSize:10, fontWeight:600, background:tc2.bg, color:tc2.color, padding:'1px 7px', borderRadius:20, marginLeft:'auto' }}>{s.type_seance}</span>
                          </div>
            ...
            {detailTab==='notes' && !editingDetail && (
              editingDetail ? null : detail.notes ? (
                ...
                <div>
                  {editingDetail ? (
                    <textarea value={editForm.notes} onChange={e=>setEditForm(f=>({...f,notes:e.target.value}))} rows={4} style={{ ...mInp, resize:'vertical', fontFamily:'inherit' }} />
                  ) : (
                    <div style={{ textAlign:'center', padding:'30px', color:'var(--color-text-secondary)', fontSize:13 }}>
                      <i className="ti ti-notes-off" style={{ fontSize:28, display:'block', marginBottom:8 }} />Aucune note pour cette séance
                    </div>
                  )}
                </div>
              )
            ...
            <div style={{ display:'flex', gap:8, marginTop:20, paddingTop:16, borderTop:'0.5px solid var(--color-border-tertiary)' }}>
              {editingDetail ? (
                ...
                <>
                  <button onClick={() => deleteSeance(detail.id)} style={{ padding:'8px 14px', borderRadius:8, border:'0.5px solid #FBEAF0', background:'transparent', color:'#993556', cursor:'pointer', fontSize:13 }}>
...
function Btn({ icon, label, onClick, secondary }) {
  return <button onClick={onClick} style={{ display:'flex', alignItems:'center', gap:6, padding:'8px 14px', borderRadius:8, cursor:'pointer', fontSize:13, fontWeight:500, border:secondary?'0.5px solid var(--color-border-secondary)':'none', background:secondary?'transparent':'var(--color-accent)', color:secondary?'var(--color-text-primary)':'#fff' }}><i className={`ti ${icon}`} style={{ fontSize:15 }} />{label}</button>
}
...
function Sel({ value, onChange, options }) {
  return <select value={value} onChange={e=>onChange(e.target.value)} style={{ padding:'7px 10px', borderRadius:8, border:'0.5px solid var(--color-border-secondary)', background:'var(--color-background-secondary)', color:'var(--color-text-primary)', fontSize:13, cursor:'pointer' }}>{options.map(([v,l])=><option key={v} value={v}>{l}</option>)}</select>
}

function Field({ label, children }) {
  return <div><div style={{ fontSize:11, fontWeight:600, color:'var(--color-text-secondary)', textTransform:'uppercase', letterSpacing:'.05em', marginBottom:5 }}>{label}</div>{children}</div>
}

function MField({ label, children, style }) {
  return <div style={style}><div style={{ fontSize:10, fontWeight:600, color:'var(--color-text-secondary)', textTransform:'uppercase', letterSpacing:'.06em', marginBottom:5 }}>{label}</div>{children}</div>
}
