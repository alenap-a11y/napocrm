...
// Types volontairement limités au massage bien-être (pas de connotation
// thérapeutique/médicale) — cf. point de vigilance légal du brief.
const TYPES = ['Relaxant', 'Californien', 'Suédois', 'Autre']
...
export default function MassageSeance() {
  const { id } = useParams()
  ...
  useEffect(() => {
    async function load() {
      const { data: s } = await supabase.from('fiches_massage')
        .select('*, clients(id, prenom, nom)').eq('id', id).single()
      if (!s) { navigate('/napo-massage'); return }
      setSeance(s)
      setClient(s.clients)
      const { data: hist } = await supabase.from('fiches_massage')
  ...
  async function handleSave() {
    setSaving(true)
    await supabase.from('fiches_massage').update({
      date_seance: seance.date_seance,
      heure_seance: seance.heure_seance,
      type_massage: seance.type_massage,
      zones_travaillees: seance.zones_travaillees,
      intensite: seance.intensite ?? null,
      bilan: seance.bilan,
  ...
  return (
    <div style={{ padding:'1.6rem 2rem', fontFamily:'inherit', maxWidth:1100 }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:'1.2rem' }}>
        <button onClick={() => navigate('/napo-massage')} style={{ background:'none', border:'none', cursor:'pointer', color:'var(--color-text-secondary)', fontSize:13, display:'flex', alignItems:'center', gap:4, padding:0 }}>
          <i className="ti ti-arrow-left" style={{ fontSize:15 }} />Retour
        </button>
        <span style={{ color:'var(--color-border-secondary)' }}>/</span>
        <i className="ti ti-hand-move" style={{ fontSize:18, color:'var(--color-accent)' }} />
        ...
        <div style={{ marginLeft:'auto', display:'flex', gap:8, alignItems:'center' }}>
          ...
          <button onClick={() => navigate(`/live/massage/${id}`)} style={{ display:'flex', alignItems:'center', gap:6, padding:'8px 18px', borderRadius:8, border:'0.5px solid var(--color-border-secondary)', background:'var(--color-background-secondary)', color:'var(--color-text-primary)', fontSize:13, fontWeight:600, cursor:'pointer' }}>
      ...
      <div style={{ background:'var(--color-background-secondary)', border:'0.5px solid var(--color-border-tertiary)', borderRadius:14, padding:'14px 16px', marginBottom:14 }}>
        ...
        <div style={{ display:'flex', gap:16, flexWrap:'wrap', marginBottom:12, alignItems:'center' }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:6 }}>
            <span style={{ fontSize:10, color:'var(--color-text-secondary)', textTransform:'uppercase', letterSpacing:'.04em' }}>Type</span>
            <select value={seance.type_massage || ''} onChange={e => updateSeanceField('type_massage', e.target.value)} style={{ ...inp, width:'auto' }}>
        ...
        <div>
          ...
          <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
            {historique.map(h => (
              <span key={h.id} onClick={() => navigate(`/napo-massage/${h.id}`)}
      ...
      {/* Zones + intensité */}
      <div style={{ background:'var(--color-background-secondary)', border:'0.5px solid var(--color-border-tertiary)', borderRadius:14, padding:'14px 16px', marginBottom:14 }}>
        <div style={{ fontSize:9, color:'var(--color-text-secondary)', textTransform:'uppercase', letterSpacing:'.05em', marginBottom:6 }}>Zones travaillées</div>
        <textarea value={seance.zones_travaillees || ''} onChange={e => updateSeanceField('zones_travaillees', e.target.value)}
          rows={2} placeholder="Dos, épaules, jambes..."
          style={{ ...inp, resize:'none', lineHeight:1.6, marginBottom:12 }} />

        <div style={{ fontSize:9, color:'var(--color-text-secondary)', textTransform:'uppercase', letterSpacing:'.05em', marginBottom:8 }}>Pression / intensité (0-8)</div>
        <div style={{ display:'flex', gap:4, flexWrap:'wrap' }}>
          {ECHELLE.map(v => (
            <button key={v} onClick={() => updateSeanceField('intensite', seance.intensite === v ? null : v)}
              style={{ width:26, height:26, borderRadius:6, fontSize:11, fontWeight:600, cursor:'pointer', border:'0.5px solid var(--color-border-secondary)', background:seance.intensite===v ? '#A05A00' : 'var(--color-background-primary)', color:seance.intensite===v ? '#fff' : 'var(--color-text-secondary)' }}>
              {v}
            </button>
          ))}
        </div>
