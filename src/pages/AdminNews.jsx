...
export default function AdminNews() {
  ...
  function startEdit(item) {
    setEditId(item.id)
    setForm({ titre: item.titre, contenu: item.contenu, publie: item.publie })
  }
  ...
  return (
    <div style={s.wrap}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
        <h1 style={s.h1}>News Naposolo</h1>
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          {msg && <span style={{ fontSize: '12px', color: '#4ade80' }}>{msg}</span>}
          <button onClick={startNew} style={{ ...s.btn, background: '#B8961E', color: '#fff' }}>+ Nouvelle news</button>
        </div>
      </div>
      ...
      {editId && (
        <div style={{ ...s.card, border: '1px solid #B8961E', marginBottom: '1.5rem' }}>
          ...
          <input style={s.input} value={form.titre} onChange={e => setForm({ ...form, titre: e.target.value })} placeholder="Titre de la news..." />
          <label style={s.label}>Contenu</label>
          <textarea style={s.textarea} value={form.contenu} onChange={e => setForm({ ...form, contenu: e.target.value })} placeholder="Contenu de la news..." />
          <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
        ...
        <div key={item.id} style={s.card}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: '10px', color: 'rgba(255,255,255,0.25)', marginTop: '6px' }}>{new Date(item.created_at).toLocaleDateString('fr-FR')}</div>
            </div>
      ...
      {items.length === 0 && !editId && (
        <div style={{ textAlign: 'center', color: 'var(--color-text-secondary)', fontSize: '13px', marginTop: '3rem' }}>
          Aucune news — clique sur "+ Nouvelle news"
        </div>
      )}
    </div>
  )
