...
export default function NaturopathieSeance() {
  const { id } = useParams()
  ...
  useEffect(() => {
    async function load() {
      const { data: s } = await supabase.from('fiches_naturopathie')
        .select('*, clients(id, prenom, nom)').eq('id', id).single()
      if (!s) { navigate('/napo-naturopathie'); return }
      setSeance(s)
      setClient(s.clients)
      const { data: hist } = await supabase.from('fiches_naturopathie')
  ...
  async function handleSave() {
    setSaving(true)
    await supabase.from('fiches_naturopathie').update({
      date_seance: seance.date_seance,
      heure_seance: seance.heure_seance,
      motif_consultation: seance.motif_consultation,
      habitudes_alimentaires: seance.habitudes_alimentaires,
  ...
  return (
    <div style={{ padding:'1.6rem 2rem', fontFamily:'inherit', maxWidth:1100 }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:'1.2rem' }}>
        <button onClick={() => navigate('/napo-naturopathie')} style={{ background:'none', border:'none', cursor:'pointer', color:'var(--color-text-secondary)', fontSize:13, display:'flex', alignItems:'center', gap:4, padding:0 }}>
          <i className="ti ti-arrow-left" style={{ fontSize:15 }} />Retour
        </button>
        <span style={{ color:'var(--color-border-secondary)' }}>/</span>
        <i className="ti ti-plant-2" style={{ fontSize:18, color:'var(--color-accent)' }} />
        ...
        <div style={{ marginLeft:'auto', display:'flex', gap:8, alignItems:'center' }}>
          ...
          <button onClick={() => navigate(`/live/naturopathie/${id}`)} style={{ display:'flex', alignItems:'center', gap:6, padding:'8px 18px', borderRadius:8, border:'0.5px solid var(--color-border-secondary)', background:'var(--color-background-secondary)', color:'var(--color-text-primary)', fontSize:13, fontWeight:600, cursor:'pointer' }}>
      ...
      <div style={{ background:'var(--color-background-secondary)', border:'0.5px solid var(--color-border-tertiary)', borderRadius:14, padding:'14px 16px', marginBottom:14 }}>
        ...
        <div>
          ...
          <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
            {historique.map(h => (
              <span key={h.id} onClick={() => navigate(`/napo-naturopathie/${h.id}`)}
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
        <div style={{ background:'var(--color-background-secondary)', border:'0.5px solid var(--color-border-tertiary)', borderRadius:14, padding:'14px 16px' }}>
          <div style={{ fontSize:9, color:'var(--color-text-secondary)', textTransform:'uppercase', letterSpacing:'.05em', marginBottom:6 }}>Motif de consultation</div>
          <textarea value={seance.motif_consultation || ''} onChange={e => updateSeanceField('motif_consultation', e.target.value)}
            rows={2} placeholder="Motif de la consultation..."
            style={{ ...inp, resize:'none', lineHeight:1.6 }} />
        </div>

        <div style={{ background:'var(--color-background-secondary)', border:'0.5px solid var(--color-border-tertiary)', borderRadius:14, padding:'14px 16px' }}>
          <div style={{ fontSize:9, color:'var(--color-text-secondary)', textTransform:'uppercase', letterSpacing:'.05em', marginBottom:6 }}>Habitudes alimentaires</div>
          <textarea value={seance.habitudes_alimentaires || ''} onChange={e => updateSeanceField('habitudes_alimentaires', e.target.value)}
            rows={3} placeholder="Alimentation, rythme, sensibilités..."
            style={{ ...inp, resize:'none', lineHeight:1.6 }} />
        </div>

        <div style={{ background:'var(--color-background-secondary)', border:'0.5px solid var(--color-border-tertiary)', borderRadius:14, padding:'14px 16px' }}>
          <div style={{ fontSize:9, color:'var(--color-text-secondary)', textTransform:'uppercase', letterSpacing:'.05em', marginBottom:6 }}>Recommandations données</div>
          <textarea value={seance.recommandations || ''} onChange={e => updateSeanceField('recommandations', e.target.value)}
            rows={3} placeholder="Hygiène de vie, plantes, compléments..."
            style={{ ...inp, resize:'none', lineHeight:1.6 }} />
        </div>
