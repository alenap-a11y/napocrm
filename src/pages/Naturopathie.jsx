...
export default function Naturopathie() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const [{ data: s }, { data: c }] = await Promise.all([
        supabase.from('fiches_naturopathie').select('*, clients(prenom, nom)').eq('user_id', user.id).order('date_seance', { ascending: false }),
  ...
  async function handleNouvelle(clientId) {
    ...
    const { data, error } = await supabase.from('fiches_naturopathie').insert({
      user_id: user.id,
      client_id: clientId,
    ...
    if (!error && data) navigate(`/napo-naturopathie/${data.id}`)
  }
    ...
    <div style={{ padding:'1.6rem 2rem', fontFamily:'inherit' }}>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'1.2rem' }}>
        <div style={{ display:'flex', alignItems:'center', gap:12 }}>
          <i className="ti ti-plant-2" style={{ fontSize:24, color:'var(--color-accent)' }} />
          <div>
            <div style={{ fontSize:22, fontWeight:600, color:'var(--color-text-primary)' }}>Napo-Naturopathie</div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:12, marginBottom:18 }}>
        <StatCard icon="ti-plant-2" iconBg="#F0FDF4" iconColor="#16A34A" label="Total séances"  value={seances.length} />
        ...
        <>
          ...
          <div style={{ background:'var(--color-background-secondary)', borderRadius:12, overflow:'hidden', border:'0.5px solid var(--color-border-tertiary)' }}>
            <div style={{ display:'grid', gridTemplateColumns:'1.5fr 1fr 80px 1fr 80px', padding:'8px 16px', borderBottom:'0.5px solid var(--color-border-tertiary)' }}>
              {['Client','Date','Séance N°','Motif',''].map((h,i) => (
            ...
            {loading ? (
              ...
              <div style={{ padding:'40px', textAlign:'center', color:'var(--color-text-secondary)', fontSize:13 }}>
                <i className="ti ti-plant-2" style={{ fontSize:28, display:'block', marginBottom:8 }} />
                Aucune séance naturopathie enregistrée
              </div>
            ) : filtered.map((s, idx) => (
              <div key={s.id} onClick={() => navigate(`/napo-naturopathie/${s.id}`)}
                ...
                <div style={{ fontSize:11, color:'var(--color-text-secondary)', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{s.motif_consultation || '—'}</div>
            ...
            {clients.length === 0 ? (
              ...
              <div key={c.id} onClick={() => handleNouvelle(c.id)}
                ...
                <div style={{ width:36, height:36, borderRadius:'50%', background:'#F0FDF4', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                  <span style={{ fontSize:13, fontWeight:700, color:'#16A34A' }}>{c.prenom[0]}{c.nom[0]}</span>
                </div>
