...
export default function SophrologieSeance() {
  const { id } = useParams()
  ...
  useEffect(() => {
    async function load() {
      const { data: s } = await supabase.from('fiches_sophrologie')
        .select('*, clients(id, prenom, nom)').eq('id', id).single()
      if (!s) { navigate('/napo-sophrologie'); return }
      setSeance(s)
      setClient(s.clients)
      const { data: hist } = await supabase.from('fiches_sophrologie')
  ...
  async function handleSave() {
    setSaving(true)
    await supabase.from('fiches_sophrologie').update({
      date_seance: seance.date_seance,
      heure_seance: seance.heure_seance,
      objectif_seance: seance.objectif_seance,
      exercices_pratiques: seance.exercices_pratiques,
      ressenti_avant: seance.ressenti_avant ?? null,
      ressenti_apres: seance.ressenti_apres ?? null,
      bilan: seance.bilan,
  ...
  return (
    <div style={{ padding:'1.6rem 2rem', fontFamily:'inherit', maxWidth:1100 }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:'1.2rem' }}>
        <button onClick={() => navigate('/napo-sophrologie')} style={{ background:'none', border:'none', cursor:'pointer', color:'var(--color-text-secondary)', fontSize:13, display:'flex', alignItems:'center', gap:4, padding:0 }}>
          <i className="ti ti-arrow-left" style={{ fontSize:15 }} />Retour
        </button>
        <span style={{ color:'var(--color-border-secondary)' }}>/</span>
        <i className="ti ti-mood-smile" style={{ fontSize:18, color:'var(--color-accent)' }} />
        ...
        <div style={{ marginLeft:'auto', display:'flex', gap:8, alignItems:'center' }}>
          ...
          <button onClick={() => navigate(`/live/sophrologie/${id}`)} style={{ display:'flex', alignItems:'center', gap:6, padding:'8px 18px', borderRadius:8, border:'0.5px solid var(--color-border-secondary)', background:'var(--color-background-secondary)', color:'var(--color-text-primary)', fontSize:13, fontWeight:600, cursor:'pointer' }}>
      ...
      <div style={{ background:'var(--color-background-secondary)', border:'0.5px solid var(--color-border-tertiary)', borderRadius:14, padding:'14px 16px', marginBottom:14 }}>
        ...
        <div>
          ...
          <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
            {historique.map(h => (
              <span key={h.id} onClick={() => navigate(`/napo-sophrologie/${h.id}`)}
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:14, marginBottom:14 }}>
        <div style={{ background:'var(--color-background-secondary)', border:'0.5px solid var(--color-border-tertiary)', borderRadius:14, padding:'14px 16px' }}>
          ...
          <textarea value={seance.objectif_seance || ''} onChange={e => updateSeanceField('objectif_seance', e.target.value)}
            rows={2} placeholder="Gestion du stress, sommeil, confiance..."
            style={{ ...inp, resize:'none', lineHeight:1.6 }} />
        </div>

        <div style={{ background:'var(--color-background-secondary)', border:'0.5px solid var(--color-border-tertiary)', borderRadius:14, padding:'14px 16px' }}>
          <div style={{ fontSize:9, color:'var(--color-text-secondary)', textTransform:'uppercase', letterSpacing:'.05em', marginBottom:6 }}>Exercices pratiqués</div>
          <textarea value={seance.exercices_pratiques || ''} onChange={e => updateSeanceField('exercices_pratiques', e.target.value)}
            rows={3} placeholder="Respiration, relaxation dynamique, visualisation..."
            style={{ ...inp, resize:'none', lineHeight:1.6 }} />
        </div>
      </div>
      ...
      <div style={{ display:'flex', gap:14, marginBottom:14 }}>
        {[['ressenti_avant','Ressenti avant'],['ressenti_apres','Ressenti après']].map(([field, label]) => (
          <div key={field} style={{ flex:1, background:'var(--color-background-secondary)', border:'0.5px solid var(--color-border-tertiary)', borderRadius:14, padding:'14px 16px' }}>
            <div style={{ fontSize:9, color:'#EA580C', textTransform:'uppercase', letterSpacing:'.05em', marginBottom:8, fontWeight:600 }}>{label} (0-8)</div>
            <div style={{ display:'flex', gap:4, flexWrap:'wrap' }}>
              {ECHELLE.map(v => (
                <button key={v} onClick={() => updateSeanceField(field, seance[field] === v ? null : v)}
                  style={{ width:26, height:26, borderRadius:6, fontSize:11, fontWeight:600, cursor:'pointer', border:'0.5px solid var(--color-border-secondary)', background:seance[field]===v ? '#EA580C' : 'var(--color-background-primary)', color:seance[field]===v ? '#fff' : 'var(--color-text-secondary)' }}>
                  {v}
                </button>
              ))}
            </div>
