...
export default function AdminFaq() {
  ...
  const [form, setForm] = useState({ question: '', reponse: '', ordre: 0, publie: false })
  ...
  async function fetchAll() {
    const { data } = await supabase.from('faq').select('*').order('ordre')
    setItems(data || [])
  }

  function startNew() {
    setEditId('new')
    setForm({ question: '', reponse: '', ordre: items.length, publie: false })
  }

  function startEdit(item) {
    setEditId(item.id)
    setForm({ question: item.question, reponse: item.reponse, ordre: item.ordre, publie: item.publie })
  }
    ...
    <div style={s.wrap}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          ...
          <button onClick={startNew} style={{ ...s.btn, background: '#B8961E', color: '#fff' }}>+ Nouvelle question</button>
        </div>
      </div>
      ...
      {editId && (
        <div style={{ ...s.card, border: '1px solid #B8961E', marginBottom: '1.5rem' }}>
          ...
          <input style={s.input} value={form.question} onChange={e => setForm({ ...form, question: e.target.value })} placeholder="Question..." />
          <label style={s.label}>Réponse</label>
          <textarea style={s.textarea} value={form.reponse} onChange={e => setForm({ ...form, reponse: e.target.value })} placeholder="Réponse..." />
          <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
      ...
      {items.map(item => (
        <div key={item.id} style={s.card}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: '13px', fontWeight: 500, color: 'var(--color-text-primary)', marginBottom: '4px' }}>{item.question}</div>
              <div style={{ fontSize: '12px', color: 'var(--color-text-secondary)', whiteSpace: 'pre-wrap' }}>{item.reponse}</div>
            </div>
      ...
      {items.length === 0 && !editId && (
        <div style={{ textAlign: 'center', color: 'var(--color-text-secondary)', fontSize: '13px', marginTop: '3rem' }}>
          Aucune question — clique sur "+ Nouvelle question"
        </div>
      )}
    </div>
  )
