...
export default function Sonotherapie() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const [{ data: s }, { data: c }] = await Promise.all([
        supabase.from('fiches_sonotherapie').select('*, clients(prenom, nom)').eq('user_id', user.id).order('date_seance', { ascending: false }),
  ...
  async function handleNouvelle(clientId) {
    ...
    const { data, error } = await supabase.from('fiches_sonotherapie').insert({
      user_id: user.id,
      client_id: clientId,
    ...
    if (!error && data) navigate(`/napo-sonotherapie/${data.id}`)
  }
    ...
    <div style={{ padding:'1.6rem 2rem', fontFamily:'inherit' }}>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'1.2rem' }}>
        <div style={{ display:'flex', alignItems:'center', gap:12 }}>
          <i className="ti ti-wave-sine" style={{ fontSize:24, color:'var(--color-accent)' }} />
          <div>
            <div style={{ fontSize:22, fontWeight:600, color:'var(--color-text-primary)' }}>Napo-Sonothérapie</div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:12, marginBottom:18 }}>
        <StatCard icon="ti-wave-sine" iconBg="#EEF2FF" iconColor="#4F46E5" label="Total séances"  value={seances.length} />
        ...
        <>
          ...
          <div style={{ background:'var(--color-background-secondary)', borderRadius:12, overflow:'hidden', border:'0.5px solid var(--color-border-tertiary)' }}>
            <div style={{ display:'grid', gridTemplateColumns:'1.5fr 1fr 80px 1fr 80px', padding:'8px 16px', borderBottom:'0.5px solid var(--color-border-tertiary)' }}>
              {['Client','Date','Séance N°','Instruments',''].map((h,i) => (
            ...
            {loading ? (
              ...
              <div style={{ padding:'40px', textAlign:'center', color:'var(--color-text-secondary)', fontSize:13 }}>
                <i className="ti ti-wave-sine" style={{ fontSize:28, display:'block', marginBottom:8 }} />
                Aucune séance sonothérapie enregistrée
              </div>
            ) : filtered.map((s, idx) => (
              <div key={s.id} onClick={() => navigate(`/napo-sonotherapie/${s.id}`)}
                ...
                <div style={{ fontSize:11, color:'var(--color-text-secondary)', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{(s.instruments_utilises || []).join(', ') || '—'}</div>
            ...
            {clients.length === 0 ? (
              ...
              <div key={c.id} onClick={() => handleNouvelle(c.id)}
                ...
                <div style={{ width:36, height:36, borderRadius:'50%', background:'#EEF2FF', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                  <span style={{ fontSize:13, fontWeight:700, color:'#4F46E5' }}>{c.prenom[0]}{c.nom[0]}</span>
                </div>
