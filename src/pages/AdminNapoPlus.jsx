...
const EMPTY = { title: '', description: '', category: 'Bien-être', icon: 'ti-sparkles', icon_bg: '#EEEDFE', icon_color: '#534AB7', category_color: '#534AB7', category_bg: '#EEEDFE', status: 'available', cta: 'Bientôt disponible', path: '', position: 0, visible: true }

export default function AdminNapoPlus() {
  ...
  async function fetchModules() {
    setLoading(true)
    const { data } = await supabase.from('napoplus_modules').select('*').order('position')
    setModules(data || [])
    setLoading(false)
  }
  ...
  async function save() {
    if (editing) {
      await supabase.from('napoplus_modules').update(form).eq('id', editing)
    } else {
      await supabase.from('napoplus_modules').insert(form)
    }
    setForm(EMPTY)
    setEditing(null)
    fetchModules()
  ...
  async function remove(id) {
    ...
    await supabase.from('napoplus_modules').delete().eq('id', id)
    fetchModules()
  }
  ...
  return (
    <div style={{ padding: 32 }}>
      <h2 style={{ fontSize: 18, fontWeight: 700, marginBottom: 24 }}>Modules Napo+</h2>

      {/* Formulaire */}
      <div style={{ background: '#F9F9FF', border: '1px solid #EEEDFE', borderRadius: 12, padding: 20, marginBottom: 32 }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          {[['title','Titre'],['description','Description'],['category','Catégorie'],['icon','Icône (ti-xxx)'],['status','Status (active/available/soon)'],['cta','Bouton CTA'],['path','Path (si active)'],['position','Position']].map(([key, label]) => (
            <div key={key}>
          ...
          <tbody>
            {modules.map(mod => (
              <tr key={mod.id} style={{ borderBottom: '1px solid #EEE' }}>
                ...
                <td style={{ padding: '10px 12px' }}><span style={{ background: mod.status === 'active' ? '#E6F4EE' : '#EEEDFE', color: mod.status === 'active' ? '#1A7A4A' : '#534AB7', padding: '2px 8px', borderRadius: 20, fontSize: 11 }}>{mod.status}</span></td>
