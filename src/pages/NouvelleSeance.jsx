...
export default function NouvelleSeance() {
  ...
  const lbl = { fontSize: 10, color: 'rgba(255,255,255,0.5)', marginBottom: 4, display: 'block', textTransform: 'uppercase', letterSpacing: '.06em' }
  const inp = { width: '100%', background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.13)', borderRadius: 3, color: '#fff', fontSize: 12, padding: '6px 8px', outline: 'none', boxSizing: 'border-box', fontFamily: 'inherit' }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', fontFamily: 'inherit', overflow: 'hidden' }}>
      ...
      {/* ── Topbar ── */}
      <div style={{ height: 44, background: '#1a2744', display: 'flex', alignItems: 'center', padding: '0 14px', gap: 10, flexShrink: 0, borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
        <button
          onClick={() => navigate('/seances')}
          style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '5px 10px', borderRadius: 3, border: '1px solid rgba(255,255,255,0.18)', background: 'transparent', color: 'rgba(255,255,255,0.7)', fontSize: 12, cursor: 'pointer' }}
        >
          <i className="ti ti-arrow-left" style={{ fontSize: 13 }} />
          Séances
        </button>
        <span style={{ flex: 1, textAlign: 'center', fontSize: 14, fontWeight: 600, color: '#fff', letterSpacing: '.02em' }}>
          Nouvelle séance
        </span>
        <button
          onClick={handleSave}
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        ...
        {/* ── GAUCHE navy ── */}
        <div style={{ width: 220, flexShrink: 0, background: '#1a2744', display: 'flex', flexDirection: 'column', overflow: 'hidden', borderRight: '1px solid rgba(255,255,255,0.07)' }}>
          <div style={{ flex: 1, overflowY: 'auto', padding: '14px 14px 0' }}>

            {/* Section Client */}
            <div style={{ fontSize: 10, fontWeight: 700, color: '#c9a84c', letterSpacing: '.1em', textTransform: 'uppercase', marginBottom: 12 }}>Client</div>
              ...
              <div style={{ display: 'flex', gap: 4 }}>
                {[['H','Homme'],['F','Femme'],['A','Autre']].map(([g, l]) => (
                  <button key={g} onClick={() => setGenre(g === genre ? '' : g)} style={{
                    ...
                    border: `1px solid ${genre === g ? '#c9a84c' : 'rgba(255,255,255,0.13)'}`,
                    background: genre === g ? '#c9a84c' : 'rgba(255,255,255,0.06)',
                    color: genre === g ? '#1a2744' : 'rgba(255,255,255,0.65)',
            ...
            {/* Section Séance */}
            <div style={{ borderTop: '1px solid rgba(255,255,255,0.09)', paddingTop: 14, marginBottom: 12 }}>
              <div style={{ fontSize: 10, fontWeight: 700, color: '#c9a84c', letterSpacing: '.1em', textTransform: 'uppercase', marginBottom: 12 }}>Séance</div>
            </div>
            ...
            <div style={gap}>
              ...
              <select style={sel} value={duree} onChange={e => setDuree(e.target.value)}>
                {[['30','30 min'],['45','45 min'],['60','1 h'],['90','1 h 30'],['120','2 h']].map(([v,l]) => (
                  <option key={v} value={v} style={{ background: '#1a2744' }}>{l}</option>
                ))}
              </select>
            </div>
            ...
            <div style={{ ...gap, marginBottom: 16 }}>
              ...
              <select style={sel} value={type} onChange={e => setType(e.target.value)}>
                {['Sophrologie','Naturopathie','Coaching','Énergie','Massage','Autre'].map(t => (
                  <option key={t} value={t} style={{ background: '#1a2744' }}>{t}</option>
                ))}
              </select>
            </div>
          </div>
          ...
          {/* Bouton Sauvegarder */}
          <div style={{ padding: '12px 14px', borderTop: '1px solid rgba(255,255,255,0.09)', flexShrink: 0 }}>
            <button
              onClick={handleSave}
              disabled={saving}
