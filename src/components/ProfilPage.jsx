...
export default function ProfilPage({ accent, onSignOut }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      } catch (e) { console.error('Erreur chargement profil :', e.message) }
      setLoading(false)
    }
    load()
  }, [])
  ...
  async function save() {
    ...
    try {
      ...
      setEditing(false); setSaveMsg('✓ Profil mis à jour')
    ...
    <div className="profil">
      <div className="pf-header">
        <div onClick={() => fileInputRef.current?.click()} title="Changer la photo"
          style={{ position: 'relative', width: 52, height: 52, borderRadius: '50%', border: `2px solid ${accent}`, cursor: 'pointer', flexShrink: 0, overflow: 'hidden' }}>
          {profil.avatar_url ? (
            <img src={profil.avatar_url} alt="avatar" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
          ) : (
            <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, #1a1a2e 0%, #16213e 100%)' }}>
              <span style={{ fontSize: 9, fontWeight: 700, color: accent, letterSpacing: 1, lineHeight: 1 }}>NAPO</span>
              <span style={{ fontSize: 15, fontWeight: 800, color: '#fff', lineHeight: 1.1 }}>{initials}</span>
            </div>
          )}
          <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: uploadingAvatar ? 1 : 0, transition: 'opacity .2s' }}
            ...
            {uploadingAvatar
              ...
              : <i className="ti ti-camera" style={{ color: '#fff', fontSize: 16 }} />}
          </div>
        </div>
          ...
          <div>
            ...
            {profil.slug && (
              <div style={{ display: 'flex', gap: 12 }}>
                <a href={'/rdv/' + profil.slug} target="_blank" rel="noreferrer" style={{ fontSize: 12, color: '#0F6E56', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 4, fontWeight: 500 }}>
        ...
        {editing ? (
          <>
            <button className="pf-btn" onClick={save} disabled={saving} style={{ background: accent, border: 'none', opacity: saving ? .7 : 1 }}>
              <i className="ti ti-check" style={{ color: '#fff' }} /><span style={{ color: '#fff', fontWeight: 600 }}>{saving ? 'Sauvegarde…' : 'Sauvegarder les modifications'}</span>
            </button>
            <button className="pf-btn" onClick={cancel} disabled={saving}>
              <i className="ti ti-x" style={{ color: 'var(--color-text-secondary)' }} /><span>Annuler</span>
            </button>
          </>
        ) : (
          <>
